...
class ReflexAgent(Agent):
    ...
    def evaluationFunction(self, currentGameState, action):
        ...
        "*** YOUR CODE HERE ***"
        # Get PacMan's current pos
        currentPos = list(successorGameState.getPacmanPosition())
        
        # List remaining food
        food = currentGameState.getFood().asList()
        
        # Pacman does not need to move
        if action == 'STOP':
            return -inf
          
        # If Ghost would move to PacMan's position, do not move to this space.
...
class MinimaxAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        def miniMax(gameState,agent,depth):
            ...
            # If no legal actions remain, terminate state #
            if not gameState.getLegalActions(agent):
                return self.evaluationFunction(gameState),0

            # If reached max ply depth, return eval #
            if depth == self.depth:
                return self.evaluationFunction(gameState),0
